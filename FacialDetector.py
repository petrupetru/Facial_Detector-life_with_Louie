...
class FacialDetector:
    ...
    def train_classifier(self, training_examples, train_labels):
        X_train, X_test, y_train, y_test = train_test_split(training_examples, train_labels, test_size = 0.15, random_state = 42)
        ...
        best_c = 0
        best_model = None
        Cs = [0.001, 0.005, 0.0075, 0.01, 0.015, 0.02, 0.025]
        for c in Cs:
    ...
    def run(self):
        ...
        for i in range(num_test_images):
            ...
            image_scores = []
            image_detections = []
            while width * resize_scale * self.params.resize_scale > self.params.min_width:
                resize_scale = resize_scale * self.params.resize_scale
                resize_width = round(width * resize_scale)
                resized_img = imutils.resize(img, width=resize_width)

                hog_descriptors = hog(resized_img, pixels_per_cell=(self.params.dim_hog_cell, self.params.dim_hog_cell),
                                      cells_per_block=(2, 2), feature_vector=False)
                #hi = exposure.rescale_intensity(hi, in_range=(0, 10))
                #hi = imutils.resize(hi, width=800)
                #cv.imshow("hog", hi)
                #cv.waitKey()
                ...
                for y in range(0, num_rows - num_cell_in_template):
                    for x in range(0, num_cols - num_cell_in_template):
                        ...
                        if score > self.params.threshold:
                            x_min = int(x * self.params.dim_hog_cell / resize_scale)
                            y_min = int(y * self.params.dim_hog_cell / resize_scale)
                            x_max = int((x * self.params.dim_hog_cell + self.params.dim_window) / resize_scale)
                            y_max = int((y * self.params.dim_hog_cell + self.params.dim_window) / resize_scale)
        ...
        np.save(self.params.solutions_path_t1 + "/detections_all_faces.npy", arr=detections, allow_pickle=True, fix_imports=True)
        np.save(self.params.solutions_path_t1 + "/scores_all_faces.npy", arr=scores, allow_pickle=True, fix_imports=True)
        np.save(self.params.solutions_path_t1 + "/file_names_all_faces.npy", arr=file_names, allow_pickle=True, fix_imports=True)
